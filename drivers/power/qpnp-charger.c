...
#ifdef CONFIG_VENDOR_SMARTISAN
static int
get_prop_backup_battery_voltage_now(struct qpnp_chg_chip *chip)
{
	union power_supply_propval ret = {0,};
...
get_prop_battery_voltage_now(struct qpnp_chg_chip *chip)
{
	...
	if (!chip->current_batt)
		batt_voltage = get_prop_backup_battery_voltage_now(chip);
	else
		batt_voltage = get_prop_main_battery_voltage_now(chip);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		...
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = get_prop_backup_battery_voltage_now(chip);
		break;
	case POWER_SUPPLY_PROP_TEMP:
